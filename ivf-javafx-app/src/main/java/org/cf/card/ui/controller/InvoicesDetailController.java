 ...
 * @since : Jan 2, 2017
 */
public class InvoicesDetailController extends BaseController {

	// getting all message from message.property file
	private static final String mainPageTitle = MessageResource.getText("mainpage.title.billing.invoices.detail");
	...
	@FXML
	private TableView<UIBillingAndInvoice> invoicesDetailTableView;
	@FXML
	...
	@FXML
	private TableColumn<UIBillingAndInvoice, Long> totalBill;
	@FXML
	private TableColumn<UIBillingAndInvoice, Long> totalPaid;
	@FXML
	private TableColumn<UIBillingAndInvoice, Long> totalBalance;
	@FXML
	private TableColumn<UIBillingAndInvoice, String> invoiceDate;
	@FXML
	private TableColumn<UIBillingAndInvoice, String> billedBy;
	@FXML
	...
	// Variables
	private ObservableList<UIBillingAndInvoice> observableList;
	private UIBillingAndInvoiceService uIBillingAndInvoiceService = new UIBillingAndInvoiceService();;
	...
	public void buildData() {
		...
		buildTableViewWithData();
		makeTextFieldNonEditable();
	}

	/**
	 * Builds the table view with data.
	 */
	private void buildTableViewWithData() {
		resetFields();
		List<BillingInvoiceDto> billingAndInvoices = uIBillingAndInvoiceService
				.getAllBillingAndInvoiesWithCoupleId(couple.getId());
		observableList = FXCollections.observableArrayList();
		if (null != billingAndInvoices && billingAndInvoices.size() > 0) {
			...
			billingAndInvoices.forEach(new Consumer<BillingInvoiceDto>() {
				int count = 1;
			...
			try {
				...
				totalBill.setCellValueFactory(new PropertyValueFactory<UIBillingAndInvoice, Long>("totalBill"));
				totalPaid.setCellValueFactory(new PropertyValueFactory<UIBillingAndInvoice, Long>("totalPaid"));
				totalBalance.setCellValueFactory(new PropertyValueFactory<UIBillingAndInvoice, Long>("totalBalance"));
				invoiceDate.setCellValueFactory(new PropertyValueFactory<UIBillingAndInvoice, String>("invoiceDate"));
				billedBy.setCellValueFactory(new PropertyValueFactory<UIBillingAndInvoice, String>("billedBy"));
				remarksColumn.setCellValueFactory(new PropertyValueFactory<UIBillingAndInvoice, String>("remarks"));
				// remarksColumn.setCellFactory(textAreaCellfactory);
				...
				remarksColumn.setCellFactory(
						new Callback<TableColumn<UIBillingAndInvoice, String>, TableCell<UIBillingAndInvoice, String>>() {
							...
							public TableCell<UIBillingAndInvoice, String> call(
								...
								return new TextAreaCell<UIBillingAndInvoice>(invoicesDetailTableView,
										login.getRoleId());
							}
						});
			...
			invoicesDetailTableView.setItems(observableList);
			...
			totalBillTextField.setText(uiTotalBill);
			totalPaidTextField.setText(uiTotalPaid);
			totalBalanceTextField.setText(uiTotalBalance);
		}
	}
	...
	private void printAction() {
		...
		new BillingInvoiceDetailAndUnpaidPerCouplePrinting().printActionForInvoicesAndUnpaidPerCouple(
				invoicesDetailTableView, womanCode, manCode, mainApp, mainPageTitle, iconURL, billingInvoiceDto);
	}
	...
	private void resetFields() {
		invoicesDetailTableView.getItems().clear();
	}
